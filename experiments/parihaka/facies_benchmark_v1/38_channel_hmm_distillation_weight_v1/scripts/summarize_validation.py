# ruff: noqa: INP001
"""Summarize validation-only Channel screening for distillation weights."""
...
BRANCHES = {
	'mae': {
		'control': 'mae',
		'variants': {
			'distill005': 'mae_hmm_k6_distill005',
			'distill010': 'mae_hmm_k6_distill010',
			'distill020': 'mae_hmm_k6',
			'distill040': 'mae_hmm_k6_distill040',
		},
	},
	'local_bt': {
		'control': 'local_barlow_twins',
		'variants': {
			'distill005': 'local_barlow_twins_hmm_k6_distill005',
			'distill010': 'local_barlow_twins_hmm_k6_distill010',
			'distill020': 'local_barlow_twins_hmm_k6',
			'distill040': 'local_barlow_twins_hmm_k6_distill040',
		},
	},
}
LAYOUTS = tuple(f'layout_{index:03d}' for index in range(5))
VARIANT_ORDER = tuple(VARIANT_DISTILLATION_WEIGHTS)
...
def _summarize_variants(
	...
	for variant in VARIANT_ORDER:
		...
		per_variant[variant] = {
			'distillation_weight': VARIANT_DISTILLATION_WEIGHTS[variant],
...
def write_report(result: Mapping[str, Any], report_root: Path) -> None:
	...
	lines = [
		'# HMM distillation-weight validation screening',
		'',
		...
		'',
		(
			'| variant | weight | branch | mean | median | sample std | wins | ties | '
			'losses | eligible |'
		),
		'|---|---:|---|---:|---:|---:|---:|---:|---:|:---:|',
	]
...
def summarize_validation(
	...
	result = {
		...
		'data_size': 'medium',
		'variant_distillation_weights': VARIANT_DISTILLATION_WEIGHTS,
...
def main(argv: Sequence[str] | None = None) -> int:
	"""Run distillation-weight validation screening from explicit artifact roots."""
