# Parihaka local Barlow Twins v1

Parihaka の未ラベル振幅 volume から、同じ物理 token の対応だけを使って
`local_barlow_twins_3d` を事前学習し、frozen full-volume token embedding を
抽出する実験です。

比較元は
`experiments/parihaka/facies_benchmark_v1/21_ssl_hmm_continuation_v1/10_stage1/barlow_twins/`
...
export LOCAL_BT_CONFIG_DIR=experiments/parihaka/facies_benchmark_v1/22_local_barlow_twins_v1
```
...
```bash
export LOCAL_BT_CHECKPOINT="$SEIS_SSL_CLUSTER_ARTIFACT_ROOT/pretraining/parihaka/facies_benchmark_v1/local_barlow_twins_v1/full_100ep/latest.pt"
...
```bash
export LOCAL_BT_METADATA="$SEIS_SSL_CLUSTER_ARTIFACT_ROOT/embeddings/parihaka/facies_benchmark_v1/local_barlow_twins_v1/overlap_x64/parihaka.embedding_metadata.json"
