...
Lightweight output is published to
`results/f3/facies_benchmark_v1/voxel_lithology_benchmark_v1/`. The publisher
allows only Markdown, JSON, CSV, and PNG products and records SHA-256 and byte
size in `publish_manifest.json`; raw `.npy`, `.pt`, and `.joblib` files remain
...
six-split suite before making the robustness interpretation.
