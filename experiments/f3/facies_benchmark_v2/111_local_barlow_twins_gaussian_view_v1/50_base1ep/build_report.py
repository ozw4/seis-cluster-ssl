# ruff: noqa: INP001
"""Build the five lightweight, validation-only base1 report projections.
...
RUNNER_SHA256 = '0181c4e9606130afd5642862d0927078326a31d5c489e65c5f3b9480d514c91b'
...
CONFIG_PATHS = {
	...
	LEGACY_ID: {
		'base': EXPERIMENT_ROOT / '10_stage1/legacy_flip_base1ep/01_matched_1ep.yaml',
		'continuation': EXPERIMENT_ROOT
		/ '15_stage2/legacy_flip_base1ep/01_continue_25ep.yaml',
		'extraction': EXPERIMENT_ROOT
		/ '20_embeddings/02_extract_legacy_flip_base1ep.yaml',
	},
}
...
def _load_runner_namespace() -> dict[str, object]:
	if RUNNER_PATH.is_symlink() or not RUNNER_PATH.is_file():
		raise FileNotFoundError(f'missing base1 validation runner: {RUNNER_PATH}')
	if file_sha256(RUNNER_PATH) != RUNNER_SHA256:
		raise ValueError('base1 validation runner SHA-256 changed')
	namespace = runpy.run_path(str(RUNNER_PATH))
	required = {
		'validation_settings_from_mapping',
		'_canonical_config',
		'validate_base1_protocol_lock',
		'validate_base1_selection_lock',
		'create_base1_final_result',
		'_expected_augmentations',
	}
	missing = required - set(namespace)
	if missing:
		raise RuntimeError(f'base1 runner API is incomplete: {sorted(missing)!r}')
	return namespace
...
def _replay_final_result(
	*,
	...
	if not callable(original_writer):
		raise TypeError('base1 runner exclusive writer is unavailable')
	final_path = cast('Path', settings.final_result)
	sentinel = final_path.with_name('.base1-report-replay-sentinel.json')
	...
	*,
	...
	for index, source_id in enumerate(SOURCE_IDS, start=1):
		...
		if not _type_sensitive_equal(
			result.get('positive_delta_count'), positive_count
		):
...
def _summary_payload(  # noqa: PLR0913
	*,
	...
	return {
		'schema_version': 1,
		'report_type': 'f3_local_barlow_twins_gaussian_base1_validation_report_v1',
	...
	lines.extend(
		[
			...
			(
				...
				'base-5 validation result; no base1 metric was used to choose '
				'the view.'
			),
			'',
		]
...
def _publish_outputs(output_dir: Path, contents: Mapping[str, str]) -> None:
	...
	with tempfile.TemporaryDirectory(
		dir=output_dir.parent, prefix='.base1-report-staging-'
	) as temporary:
...
def build_report(
	...
	*,
	...
	if config_path != DEFAULT_CONFIG.resolve():
		raise ValueError('base1 report must use the fixed validation configuration')
	...
	protocol, protocol_sha = _read_hashed_json(
		settings.protocol_lock, label='base1 protocol lock'
	)
	selection, selection_sha = _read_hashed_json(
		settings.selection_lock, label='base1 selection lock'
	)
	final_result, final_sha = _read_hashed_json(
		settings.final_result, label='base1 final result'
	)
	protocol_replayed = cast('Any', runner['validate_base1_protocol_lock'])(
		settings, canonical
	)
	if not _type_sensitive_equal(protocol, protocol_replayed):
		raise ValueError('stored protocol differs from live replay')
	selection_replayed = cast('Any', runner['validate_base1_selection_lock'])(
		settings, canonical
	)
	if not _type_sensitive_equal(selection, selection_replayed):
...
def _type_sensitive_equal(actual: object, expected: object) -> bool:
	"""Compare immutable evidence without bool/int/float aliasing."""
...
def build_parser() -> argparse.ArgumentParser:
	...
	parser = argparse.ArgumentParser(
		description='Build the immutable-evidence base1 validation report.'
	)
...
def main() -> None:
	"""Build and identify the validation-only base1 report."""
