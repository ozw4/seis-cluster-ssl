...
"""Run the reached five-base-epoch, validation-only F3 comparison.

This duration branch is authorized only by the immutable failed 25-epoch
...
PARENT_FINAL_RESULT_SCHEMA_VERSION = 2
PARENT_FINAL_RESULT_TYPE = 'f3_local_barlow_twins_gaussian25_final_validation_v1'
PARENT_SELECTION_SCHEMA_VERSION = 3
PARENT_SELECTION_TYPE = 'f3_local_barlow_twins_gaussian25_selection_v1'
PARENT_SELECTED_ID = 'local_barlow_twins_gaussian_noise_std010'
...
PROTOCOL_LOCK_TYPE = 'f3_local_barlow_twins_gaussian_base5_protocol_v1'
SELECTION_LOCK_SCHEMA_VERSION = 1
SELECTION_LOCK_TYPE = 'f3_local_barlow_twins_gaussian_base5_selection_v1'
FINAL_RESULT_SCHEMA_VERSION = 1
FINAL_RESULT_TYPE = 'f3_local_barlow_twins_gaussian_base5_final_validation_v1'
...
PARENT_FINAL_RESULT_RELATIVE_PATH = Path(
	...
	'validation/gaussian25_final_result.json'
)
PARENT_SELECTION_RELATIVE_PATH = Path(
	'f3_lithology_benchmark/local_barlow_twins_gaussian_view_v1/'
	'validation/gaussian25_selection_lock.json'
)
BRANCH_VALIDATION_RELATIVE_ROOT = Path(
	'f3_lithology_benchmark/local_barlow_twins_gaussian_view_v1/base5ep/validation'
)
EXPECTED_BASE_RELATIVE_PATHS = {
	SELECTED_ID: Path(
		'pretraining/f3/facies_benchmark_v1/'
		'local_barlow_twins_gaussian_view_v1/base5ep/stage1/'
		'gaussian_noise_std010_base5ep/full_5ep/latest.pt'
	),
	LEGACY_ID: Path(
		'pretraining/f3/facies_benchmark_v1/'
		'local_barlow_twins_gaussian_view_v1/base5ep/stage1/'
		'legacy_flip_base5ep/full_5ep/latest.pt'
	),
}
EXPECTED_FINAL_RELATIVE_PATHS = {
	SELECTED_ID: Path(
		'pretraining/f3/facies_benchmark_v1/'
		'local_barlow_twins_gaussian_view_v1/base5ep/stage2/'
		'gaussian_noise_std010_base5ep/local_bt_continue/full_25ep/latest.pt'
	),
	LEGACY_ID: Path(
		'pretraining/f3/facies_benchmark_v1/'
		'local_barlow_twins_gaussian_view_v1/base5ep/stage2/'
		'legacy_flip_base5ep/local_bt_continue/full_25ep/latest.pt'
	),
}
EXPECTED_EMBEDDING_RELATIVE_PATHS = {
	SELECTED_ID: Path(
		...
		'local_barlow_twins_gaussian_noise_std010_base5ep/overlap_x64'
	),
	LEGACY_ID: Path(
		'embeddings/f3/facies_benchmark_v2/'
		'local_barlow_twins_gaussian_view_v1/base5ep/'
		'local_barlow_twins_legacy_flip_base5ep/overlap_x64'
	),
}
	...
	def source_by_id(self, source_id: str) -> CandidateSource:
		...
		for source in self.sources:
			if source.candidate_id == source_id:
				return source
		raise ValueError(
			f'unknown base5 source {source_id!r}; expected {EXPECTED_SOURCE_IDS!r}'
		)
...
def validation_settings_from_mapping(  # noqa: C901
	...
	if settings.protocol_lock != (
		settings.runs_root.parent / 'gaussian_base5_protocol_lock.json'
	):
		raise ValueError('protocol lock must be gaussian_base5_protocol_lock.json')
	if settings.selection_lock != (
		settings.runs_root.parent / 'gaussian_base5_selection_lock.json'
	):
		raise ValueError('selection lock must be gaussian_base5_selection_lock.json')
	if settings.final_result != (
		settings.runs_root.parent / 'gaussian_base5_final_result.json'
	):
		raise ValueError('final result must be gaussian_base5_final_result.json')
	return settings
...
def _validate_branch_paths(
	...
	if (
		...
		raise ValueError('parent result path is not the pinned Gaussian25 result')
	validation_root = artifact_root / BRANCH_VALIDATION_RELATIVE_ROOT
	if settings.runs_root != validation_root / 'runs':
		raise ValueError('runs_root must use the isolated base5ep validation root')
...
def validate_parent_result(  # noqa: C901
	...
	if settings.parent_final_result != expected_result:
		raise ValueError(
			'configured parent result is not the canonical 25-epoch result'
		)
	parent, parent_sha = _read_hashed_json(
		settings.parent_final_result, label='parent Gaussian25 final result'
	)
	if parent_sha != settings.parent_final_result_sha256:
		raise ValueError('parent Gaussian25 final-result SHA-256 changed')
	...
	for key, expected in required_values.items():
		if parent.get(key) != expected:
			raise ValueError(
				f'parent Gaussian25 final result {key} must equal {expected!r}'
			)
	parent_selection = _mapping(parent, 'selection_lock')
	_require_exact_keys(
		parent_selection,
		{'path', 'selected_candidate_id', 'sha256'},
	...
	selection, selection_sha = _read_hashed_json(
		selection_path, label='parent Gaussian25 selection lock'
	)
	if selection_sha != _sha256_value(
	...
	for key, expected in selection_values.items():
		if selection.get(key) != expected:
			raise ValueError(
				f'parent Gaussian25 selection lock {key} must equal {expected!r}'
			)
	return {
...
def audit_candidate_base_checkpoint(
	*,
	...
	"""Audit one fresh five-epoch base before either continuation starts."""
	...
	return {
		...
		'audit_type': 'base5_pre_continuation_checkpoint_only',
...
def _validate_candidate_base_checkpoint(
	...
	*,
	...
	if dict(_mapping(config, 'augmentations')) != _expected_augmentations(candidate):
...
def audit_candidate_checkpoints(
	*,
	...
	"""Audit a fresh five-epoch base and exact fresh 25-epoch continuation."""
	...
	return {
		'audit_schema_version': CHECKPOINT_AUDIT_SCHEMA_VERSION,
		'audit_type': 'base5_pre_extraction_lineage_checkpoint_only',
		'candidate_id': candidate.candidate_id,
		'base_checkpoint': str(candidate.base_checkpoint),
...
def _validate_candidate_final_checkpoint(
	...
	*,
	...
	if _final_parity_projection(config) != _final_parity_projection(reference_config):
		raise ValueError(
...
def _common_checkpoint_identity(
	...
	*,
	...
	if dict(_mapping(payload, 'training_state')) != dict(training_state):
	...
	if payload.get('trained_parameter_prefixes') != EXPECTED_TRAINED_PARAMETER_PREFIXES:
...
def create_base5_protocol_lock(
	settings: ValidationSettings,
...
def validate_base5_protocol_lock(
	settings: ValidationSettings, canonical: F3FiveWayConfig
) -> Mapping[str, object]:
	"""Replay the immutable base5 protocol against all live frozen inputs."""
	stored = _read_regular_json(settings.protocol_lock, label='base5 protocol lock')
	...
	if dict(stored) != expected:
		raise ValueError('base5 protocol lock differs from live frozen inputs')
	return stored
...
def _protocol_lock_payload(  # noqa: PLR0913
	*,
	...
	if [row.get('candidate_id') for row in base_inputs] != list(EXPECTED_SOURCE_IDS):
		raise ValueError('protocol must bind both base5 arms in fixed order')
	return {
		'schema_version': PROTOCOL_LOCK_SCHEMA_VERSION,
		'protocol_lock_type': PROTOCOL_LOCK_TYPE,
		'validation_only': True,
		'stage_boundary': 'completed_fresh_base5_before_continuation',
		'base_pretraining_epochs': BASE_PRETRAINING_EPOCHS,
		'continuation_epochs': CONTINUATION_EPOCHS,
...
def _reject_pre_selection_evidence(settings: ValidationSettings) -> None:
	if _path_contains_evidence(settings.final_result) or _path_contains_evidence(
		...
		raise ValueError('selection lock must precede all base5 validation evidence')
	for source in settings.sources:
...
def _protocol_identity(
	...
	if protocol.get('protocol_lock_type') != PROTOCOL_LOCK_TYPE:
		raise ValueError('base5 protocol lock type changed')
	return {
...
def create_base5_selection_lock(
	settings: ValidationSettings,
	canonical: F3FiveWayConfig,
	*,
	created_at_utc: str | None = None,
	...
	protocol = validate_base5_protocol_lock(settings, canonical)
...
def validate_base5_selection_lock(
	settings: ValidationSettings, canonical: F3FiveWayConfig
) -> Mapping[str, object]:
	"""Replay the inherited selection without reading any base5 metric."""
	protocol = validate_base5_protocol_lock(settings, canonical)
	stored = _read_regular_json(settings.selection_lock, label='base5 selection lock')
	...
	if dict(stored) != expected:
		raise ValueError('base5 selection lock differs from inherited parent choice')
	return stored
...
def _selection_identity(
	...
	if selection.get('selection_lock_type') != SELECTION_LOCK_TYPE:
		raise ValueError('base5 selection lock type changed')
	return {
...
def audit_candidate_source(  # noqa: PLR0913
	*,
	...
	"""Audit the complete checkpoint/extraction lineage for one base5 arm."""
	...
	return {
		'audit_schema_version': CHECKPOINT_AUDIT_SCHEMA_VERSION,
		'audit_type': 'base5_validation_source_lineage',
		'candidate_id': candidate.candidate_id,
...
def enforce_validation_order(
	*,
	...
	if candidate.candidate_id not in EXPECTED_SOURCE_IDS:
		raise ValueError('only the two fixed base5 arms are admissible')
	protocol = validate_base5_protocol_lock(settings, canonical)
	selection = validate_base5_selection_lock(settings, canonical)
	if data_size == 'medium':
		return {
...
def _validation_order_provenance(
	...
	if len(identities) != 15:
		raise ValueError('base5 medium gate must bind ten arm and five random cells')
	result['medium_gate'] = {
...
def _validate_medium_random_gate(
	*,
	...
	if require_open and not gate_open:
		raise ValueError(
			'small/large validation is forbidden because neither base5 arm has '
...
def _read_candidate_job_evidence(
	*,
	...
	if audit.get('validation_order_provenance') != expected_validation_order:
...
def create_base5_final_result(
	...
	*,
	...
	expected_cells = _expected_base5_cells(medium_gate_open=gate_open)
	...
	payload = {
		...
		'authorized_next_base_pretraining_epochs': None if passed else 1,
		'failure_stage': (
...
def _expected_base5_cells(*, medium_gate_open: bool) -> set[tuple[str, str, str]]:
			...
			for data_size in DATA_SIZES:
				...
				for path in (job.metrics_path, job.output_dir / AUDIT_NAME):
					...
					if expected and not path.is_file():
						raise FileNotFoundError(
							f'missing expected base5 validation evidence: {path}'
						)
					if not expected and path.exists():
						raise ValueError(
							'base5 validation evidence exists outside reached set: '
							f'{path}'
						)
...
def build_parser() -> argparse.ArgumentParser:
	...
	parser = argparse.ArgumentParser(
		description='Run one validation-only five-base-epoch F3 job.'
	)
...
def main() -> None:  # noqa: PLR0911
	...
	if args.create_protocol_lock:
		_print_mapping(create_base5_protocol_lock(settings, canonical))
		return
	if args.create_selection_lock:
		_print_mapping(create_base5_selection_lock(settings, canonical))
		return
	protocol = validate_base5_protocol_lock(settings, canonical)
	selection = validate_base5_selection_lock(settings, canonical)
	protocol_identity = _protocol_identity(settings, protocol)
	...
	if args.create_final_result:
		_print_mapping(create_base5_final_result(settings, canonical))
		return
	candidate = settings.source_by_id(args.candidate)
