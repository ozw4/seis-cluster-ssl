...
RUNNER_SHA256 = 'c39e1ddd422af8680bd7253822549df39c48aff4461965213b19d9db54f2583e'
DEFAULT_CONFIG = EXPERIMENT_ROOT / '30_validation/01_candidate.yaml'
REPORT_OUTPUT_DIR = (
	REPOSITORY_ROOT
	/ 'reports/f3/facies_benchmark_v2/'
	'local_barlow_twins_trace_drop_view_v1/base1ep'
)
REPORT_FILENAMES = (
	'attempts.csv',
...
CONTROL_IDS = (GAUSSIAN_CONTROL_ID, LEGACY_CONTROL_ID)
...
CONFIG_PATHS = {
	...
	'extraction': (
		EXPERIMENT_ROOT
		/ '20_embeddings/01_extract_horizontal_trace_drop_p001_base1ep.yaml'
	),
}
...
def _evidence_rows(
	...
	if len(controls) != 10 or control_cells != expected_controls:
		raise ValueError('frozen controls must be exactly ten parent medium cells')
...
def _source_role_and_origin(source_id: str) -> tuple[str, str]:
	values = {
		CANDIDATE_ID: ('preregistered_trace_drop_view', 'live_trace_drop'),
		RANDOM_ID: ('canonical_random', 'protocol_frozen_random'),
		GAUSSIAN_CONTROL_ID: (
			'frozen_gaussian_control',
			'frozen_parent_base1_control',
		),
		LEGACY_CONTROL_ID: (
			'frozen_legacy_control',
			'frozen_parent_base1_control',
		),
	}
	try:
...
def _paired_rows(
	...
	comparisons = (
		(CANDIDATE_ID, RANDOM_ID, 'trace_drop_p001_minus_random'),
		(CANDIDATE_ID, GAUSSIAN_CONTROL_ID, 'trace_drop_p001_minus_gaussian'),
		(CANDIDATE_ID, LEGACY_CONTROL_ID, 'trace_drop_p001_minus_legacy'),
	)
	rows: list[dict[str, object]] = []
	for layout_id in LAYOUT_IDS:
...
def _summary_payload(  # noqa: PLR0913
	*,
	...
	return {
		'schema_version': 1,
		'report_type': 'f3_local_barlow_twins_trace_drop_p001_validation_report_v1',
		...
		'authorizes_trace_drop_p002_followup': final_result[
			'authorizes_trace_drop_p002_followup'
		],
		'authorized_trace_drop_probability': final_result[
			'authorized_trace_drop_probability'
		],
		'source_note': (
			'Human-readable validation projection only; artifacts and immutable '
			'protocol/final result remain the scientific source of truth.'
		),
	}
...
def _summary_markdown(summary: Mapping[str, object]) -> str:
	...
	return '\n'.join(
		[
			'# F3 Barlow Twins trace-drop p=.01 validation',
			'',
			(
			...
			'## Attempt',
			'',
			'| Medium mean | Random mean | Mean delta | 15/15 |',
			'| ---: | ---: | ---: | --- |',
			(
			...
			'',
			(
				'Gaussian and legacy rows are frozen parent medium controls for '
