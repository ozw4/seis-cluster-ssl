...
"""Run the authorized p=.02 trace-drop, validation-only F3 comparison.
...
random cells before any candidate metric exists. The failed p=.01 result
authorizes this follow-up and supplies its five medium cells as the sole
attribution control; only the frozen random baseline gates the candidate.
"""
...
CANDIDATE_ROLE = 'authorized_trace_drop_strength_followup'
P001_CONTROL_ID = 'local_barlow_twins_horizontal_trace_drop_p001_base1ep'
...
PARENT_PROTOCOL_LOCK_RELATIVE_PATH = Path(
	...
	'base1ep/validation/trace_drop_p001_protocol_lock.json'
)
PARENT_FINAL_RESULT_RELATIVE_PATH = Path(
	'f3_lithology_benchmark/local_barlow_twins_trace_drop_view_v1/'
	'base1ep/validation/trace_drop_p001_final_result.json'
)
EXPECTED_BASE_RELATIVE_PATH = Path(
	'pretraining/f3/facies_benchmark_v1/local_barlow_twins_trace_drop_p002_view_v1/'
	'base1ep/stage1/horizontal_trace_drop_p002_base1ep/full_1ep/latest.pt'
)
EXPECTED_FINAL_RELATIVE_PATH = Path(
	'pretraining/f3/facies_benchmark_v1/local_barlow_twins_trace_drop_p002_view_v1/'
	'base1ep/stage2/horizontal_trace_drop_p002_base1ep/'
	'local_bt_continue/full_25ep/latest.pt'
)
EXPECTED_EMBEDDING_RELATIVE_PATH = Path(
	'embeddings/f3/facies_benchmark_v2/local_barlow_twins_trace_drop_p002_view_v1/'
	'base1ep/local_barlow_twins_horizontal_trace_drop_p002_base1ep/overlap_x64'
)
...
PROTOCOL_LOCK_TYPE = (
	'f3_local_barlow_twins_horizontal_trace_drop_p002_protocol_v1'
)
...
def validation_settings_from_mapping(
	...
	if candidate_value.get('candidate_id') != CANDIDATE_ID:
		raise ValueError('candidate.candidate_id is not the preregistered p=.02 arm')
	if candidate_value.get('role') != CANDIDATE_ROLE:
		raise ValueError(
			'candidate.role must identify the authorized p=.02 follow-up'
		)
...
def _validate_experiment_paths(
	...
	if settings.parent_final_result != (
		...
		raise ValueError('parent result path is not the pinned p=.01 result')
	...
	if settings.protocol_lock != validation_root / 'trace_drop_p002_protocol_lock.json':
		raise ValueError('protocol lock path changed')
	if settings.final_result != validation_root / 'trace_drop_p002_final_result.json':
...
def validate_parent_result(  # noqa: C901, PLR0912
	...
	"""Pin the failed p=.01 result and replay its authorization evidence."""
	parent, parent_sha = _read_hashed_json(
		settings.parent_final_result, label='parent trace-drop p=.01 final result'
	)
	if parent_sha != settings.parent_final_result_sha256:
		raise ValueError('parent trace-drop p=.01 final-result SHA-256 changed')
	expected_values: Mapping[str, object] = {
		...
		'base_pretraining_epochs': 1,
		'continuation_epochs': 25,
		'candidate_id': P001_CONTROL_ID,
		'augmentations': P001_AUGMENTATIONS,
		'passed': False,
	...
	if parent_protocol.get('sha256') != PARENT_PROTOCOL_LOCK_SHA256:
		raise ValueError('parent p=.01 protocol-lock SHA-256 changed')
	if parent_protocol.get('path') != str(
		canonical.artifact_root / PARENT_PROTOCOL_LOCK_RELATIVE_PATH
	):
		raise ValueError('parent p=.01 protocol-lock path changed')
	_, live_parent_protocol_sha = _read_hashed_json(
		canonical.artifact_root / PARENT_PROTOCOL_LOCK_RELATIVE_PATH,
		label='parent trace-drop p=.01 protocol lock',
	)
	if live_parent_protocol_sha != PARENT_PROTOCOL_LOCK_SHA256:
		raise ValueError('parent p=.01 protocol-lock file changed')
	...
	expected_control_cells = {
		(P001_CONTROL_ID, layout_id, 'medium') for layout_id in LAYOUT_IDS
	}
	if (
		len(controls) != 5
		or {_cell_key(row) for row in controls} != expected_control_cells
	):
		raise ValueError('parent does not contain the exact five p=.01 medium cells')
	_validate_frozen_control_inputs(controls)
	inherited_controls = _mapping_rows(
		parent.get('frozen_medium_control_inputs'),
		'parent frozen_medium_control_inputs',
	)
	expected_inherited_cells = {
		(source_id, layout_id, 'medium')
		for source_id in (GAUSSIAN_CONTROL_ID, LEGACY_CONTROL_ID)
		for layout_id in LAYOUT_IDS
	}
	if (
		len(inherited_controls) != 10
		or {_cell_key(row) for row in inherited_controls}
		!= expected_inherited_cells
	):
		raise ValueError('parent inherited Gaussian/legacy control cells changed')
	_validate_frozen_control_inputs(inherited_controls)
	...
	scores = _scores_from_evidence((*controls, *parent_random, *inherited_controls))
	arm_results = _mapping(parent, 'arm_results')
	recomputed_arm = cast(
		'dict[str, object]',
		_shared('_arm_random_result')(
			scores, arm_id=P001_CONTROL_ID, medium_gate_open=False
		),
	)
	if not _type_sensitive_equal(arm_results.get(P001_CONTROL_ID), recomputed_arm):
		raise ValueError('parent p=.01 arm result does not recompute')
	paired_deltas = {
		layout_id: (
			scores[P001_CONTROL_ID][(layout_id, 'medium')]
	...
	if not _type_sensitive_equal(
		...
		raise ValueError('parent p=.01 medium paired deltas do not recompute')
	parent_attribution = _mapping(parent, 'trace_drop_attribution')
	recomputed_attribution = {
		'trace_minus_gaussian': _medium_control_contrast(
			scores,
			left_id=P001_CONTROL_ID,
	...
	if not _type_sensitive_equal(parent_attribution, recomputed_attribution):
		raise ValueError('parent p=.01 attribution does not recompute')
	return {
		...
		'frozen_medium_control_ids': [P001_CONTROL_ID],
		...
		'recomputed_parent_trace_drop_attribution': dict(parent_attribution),
...
def _protocol_lock_payload(  # noqa: PLR0913
	*,
	...
	return {
		...
		'selection_basis': 'parent_p001_failure_authorized_fixed_unlabeled_p002_v1',
...
def create_trace_drop_final_result(
	...
	*,
	...
	attribution = {
		'p002_minus_p001': _medium_control_contrast(
			attribution_scores,
			left_id=CANDIDATE_ID,
			right_id=P001_CONTROL_ID,
			contrast_id='trace_drop_p002_minus_trace_drop_p001_base1',
		),
	}
	...
	payload = {
		...
		'frozen_control_provenance': {
			...
			'control_candidate_ids': [P001_CONTROL_ID],
			'data_size': 'medium',
