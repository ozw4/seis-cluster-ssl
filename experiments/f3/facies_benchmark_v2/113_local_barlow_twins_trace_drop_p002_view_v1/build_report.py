# ruff: noqa: INP001
"""Build five human-only projections of the immutable trace-drop result."""
...
RUNNER_SHA256 = '724349f747c28b1cd25d36b1f5c6c7af99b6404cbcc36d7812a8ea42ca61b24c'
DEFAULT_CONFIG = EXPERIMENT_ROOT / '30_validation/01_candidate.yaml'
REPORT_OUTPUT_DIR = (
	REPOSITORY_ROOT
	/ 'reports/f3/facies_benchmark_v2/'
...
CONFIG_PATHS = {
	'base': (
		EXPERIMENT_ROOT
		/ '10_stage1/horizontal_trace_drop_p002_base1ep/01_screen_1ep.yaml'
	),
	'continuation': (
		EXPERIMENT_ROOT
		/ '15_stage2/horizontal_trace_drop_p002_base1ep/01_continue_25ep.yaml'
	),
	'extraction': (
		EXPERIMENT_ROOT
		/ '20_embeddings/01_extract_horizontal_trace_drop_p002_base1ep.yaml'
	),
}
...
ATTEMPT_FIELDS = (
	...
	'medium_p001_macro_f1_mean',
	'medium_delta_vs_p001_mean',
...
def _load_runner_namespace() -> dict[str, object]:
	if RUNNER_PATH.is_symlink() or not RUNNER_PATH.is_file():
		raise FileNotFoundError(f'missing trace-drop validation runner: {RUNNER_PATH}')
	if file_sha256(RUNNER_PATH) != RUNNER_SHA256:
		raise ValueError('trace-drop validation runner SHA-256 changed')
	namespace = runpy.run_path(str(RUNNER_PATH))
	required = {
		'validation_settings_from_mapping',
		'_canonical_config',
		'validate_trace_drop_protocol_lock',
		'create_trace_drop_final_result',
	}
	missing = required - set(namespace)
	if missing:
		raise RuntimeError(f'trace-drop runner API is incomplete: {sorted(missing)!r}')
	return namespace
...
def _replay_final_result(
	*,
	...
	creator = cast('Any', runner['create_trace_drop_final_result'])
	...
	if not callable(original_writer):
		raise TypeError('trace-drop runner exclusive writer is unavailable')
	final_path = cast('Path', settings.final_result)
	sentinel = final_path.with_name('.trace-drop-report-replay-sentinel.json')
...
def _evidence_rows(
	...
	if len(controls) != 5 or control_cells != expected_controls:
		raise ValueError('frozen controls must be exactly five p=.01 medium cells')
...
def _source_role_and_origin(source_id: str) -> tuple[str, str]:
	values = {
		CANDIDATE_ID: (
			'authorized_trace_drop_strength_followup',
			'live_trace_drop_p002',
		),
		RANDOM_ID: ('canonical_random', 'protocol_frozen_random'),
		P001_CONTROL_ID: (
			'frozen_trace_drop_p001_control',
			'frozen_parent_p001_medium_control',
		),
	}
	try:
...
def _collect_attempt_rows(
	*,
	...
	if len(medium) != 5:
		raise ValueError('trace-drop report must contain five medium cells')
	p001_medium = [
		row
		for row in validation_rows
		if row['source_id'] == P001_CONTROL_ID and row['data_size'] == 'medium'
	]
	if len(p001_medium) != 5:
		raise ValueError('trace-drop report must contain five p=.01 control cells')
	p001_by_layout = {
		cast('str', row['layout_id']): float(row['macro_f1']) for row in p001_medium
	}
	...
	return [
		{
			...
			'medium_p001_macro_f1_mean': fmean(p001_by_layout.values()),
			'medium_delta_vs_p001_mean': fmean(
				float(row['macro_f1'])
				- p001_by_layout[cast('str', row['layout_id'])]
				for row in medium
			),
...
def _paired_rows(
	...
	comparisons = (
		(CANDIDATE_ID, RANDOM_ID, 'trace_drop_p002_minus_random'),
		(CANDIDATE_ID, P001_CONTROL_ID, 'trace_drop_p002_minus_p001'),
	)
	rows: list[dict[str, object]] = []
	for layout_id in LAYOUT_IDS:
...
def _summary_payload(  # noqa: PLR0913
	*,
	...
	return {
		'schema_version': 1,
		'report_type': 'f3_local_barlow_twins_trace_drop_p002_validation_report_v1',
		...
		'trace_drop_attribution': final_result['trace_drop_attribution'],
		'passed': final_result['passed'],
		'winner_candidate_id': final_result['winner_candidate_id'],
		'failure_stage': final_result['failure_stage'],
		'authorizes_additional_trace_drop_followup': final_result[
			'authorizes_additional_trace_drop_followup'
		],
		'authorized_additional_trace_drop_probability': final_result[
			'authorized_additional_trace_drop_probability'
		],
		'source_note': (
			'Human-readable validation projection only; artifacts and immutable '
			'protocol/final result remain the scientific source of truth. Earlier '
			'Gaussian, legacy, and p=.01 history remains in the p=.01 parent report.'
		),
	}
...
def _summary_markdown(summary: Mapping[str, object]) -> str:
	...
	return '\n'.join(
		[
			...
			'',
			(
				'| p=.02 mean | Random mean | Delta vs random | p=.01 mean | '
				'Delta vs p=.01 | 15/15 |'
			),
			'| ---: | ---: | ---: | ---: | ---: | --- |',
			(
				...
				f'{float(attempt["medium_p001_macro_f1_mean"]):.9f} | '
				f'{float(attempt["medium_delta_vs_p001_mean"]):+.9f} | '
				f'{attempt["wins_all_15_over_random"]} |'
			),
			'',
			(
				'The p=.01 rows are frozen parent medium controls for direct '
...
def _publish_outputs(output_dir: Path, contents: Mapping[str, str]) -> None:
	...
	with tempfile.TemporaryDirectory(
		dir=output_dir.parent, prefix='.trace-drop-report-staging-'
	) as temporary:
...
def build_report(
	...
	*,
	...
	if config_path != DEFAULT_CONFIG.resolve():
		raise ValueError('trace-drop report must use the fixed validation config')
	...
	final_result, final_sha = _read_hashed_json(
		settings.final_result, label='trace-drop final result'
	)
	protocol_replayed = cast('Any', runner['validate_trace_drop_protocol_lock'])(
		settings, canonical
	)
...
def build_parser() -> argparse.ArgumentParser:
	...
	parser = argparse.ArgumentParser(
		description='Build the immutable-evidence trace-drop validation report.'
	)
...
def main() -> None:
	"""Build and identify the human-only trace-drop report."""
