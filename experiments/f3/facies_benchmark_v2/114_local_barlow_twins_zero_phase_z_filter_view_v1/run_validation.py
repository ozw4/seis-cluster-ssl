...
"""Run one zero-phase Z-filter, validation-only F3 comparison.
...
random cells before any candidate metric exists. The closed p=.02 trace-drop
branch supplies its five medium cells solely for attribution; only the frozen
random baseline gates the independently preregistered candidate.
"""
...
GAUSSIAN25_RUNNER = (
	EXPERIMENT_ROOT.parent / '111_local_barlow_twins_gaussian_view_v1/run_validation.py'
)
...
CANDIDATE_ROLE = 'separately_preregistered_zero_phase_z_bandwidth_view_followup'
P002_CONTROL_ID = 'local_barlow_twins_horizontal_trace_drop_p002_base1ep'
RANDOM_ID = 'random'
ZERO_PHASE_Z_FILTER_POLICY = 'horizontal_flip_zero_phase_z_filter_v1'
EXPECTED_AUGMENTATIONS = {
	'policy': ZERO_PHASE_Z_FILTER_POLICY,
...
PARENT_FINAL_RESULT_TYPE = (
	'f3_local_barlow_twins_horizontal_trace_drop_p002_final_validation_v1'
)
PARENT_PROTOCOL_LOCK_SHA256 = (
	'e33312550fb5223093c5a72b0d897afc87335067c84971d27ff4c5c0640ce727'
)
PARENT_PROTOCOL_LOCK_RELATIVE_PATH = Path(
	'f3_lithology_benchmark/local_barlow_twins_trace_drop_p002_view_v1/'
	'base1ep/validation/trace_drop_p002_protocol_lock.json'
)
PARENT_FINAL_RESULT_RELATIVE_PATH = Path(
	'f3_lithology_benchmark/local_barlow_twins_trace_drop_p002_view_v1/'
	'base1ep/validation/trace_drop_p002_final_result.json'
)
EXPECTED_BASE_RELATIVE_PATH = Path(
	'pretraining/f3/facies_benchmark_v1/local_barlow_twins_zero_phase_z_filter_view_v1/'
	'base1ep/stage1/zero_phase_z_filter_w025_base1ep/full_1ep/latest.pt'
)
EXPECTED_FINAL_RELATIVE_PATH = Path(
	'pretraining/f3/facies_benchmark_v1/local_barlow_twins_zero_phase_z_filter_view_v1/'
	'base1ep/stage2/zero_phase_z_filter_w025_base1ep/'
	'local_bt_continue/full_25ep/latest.pt'
)
EXPECTED_EMBEDDING_RELATIVE_PATH = Path(
	'embeddings/f3/facies_benchmark_v2/local_barlow_twins_zero_phase_z_filter_view_v1/'
	'base1ep/local_barlow_twins_zero_phase_z_filter_w025_base1ep/overlap_x64'
)
VALIDATION_RELATIVE_ROOT = Path(
	'f3_lithology_benchmark/local_barlow_twins_zero_phase_z_filter_view_v1/'
	'base1ep/validation'
)

CHECKPOINT_AUDIT_SCHEMA_VERSION = 1
PROTOCOL_LOCK_SCHEMA_VERSION = 1
PROTOCOL_LOCK_TYPE = 'f3_local_barlow_twins_zero_phase_z_filter_protocol_v1'
FINAL_RESULT_SCHEMA_VERSION = 1
FINAL_RESULT_TYPE = 'f3_local_barlow_twins_zero_phase_z_filter_final_validation_v1'
...
class CandidateSource:
	"""The one preregistered zero-phase Z-filter source."""
	...
	view_policy: str = ZERO_PHASE_Z_FILTER_POLICY
...
class ValidationSettings:
	...
	def source_by_id(self, source_id: str) -> CandidateSource:
		...
		if source_id != CANDIDATE_ID:
			raise ValueError(f'unknown zero-phase Z-filter candidate: {source_id!r}')
		return self.candidate
...
def validation_settings_from_mapping(
	...
	if candidate_value.get('candidate_id') != CANDIDATE_ID:
		raise ValueError('candidate.candidate_id is not the preregistered Z-filter arm')
	if candidate_value.get('role') != CANDIDATE_ROLE:
		raise ValueError('candidate.role must identify the separate Z-filter follow-up')
	...
	if parent_sha != PARENT_FINAL_RESULT_SHA256:
		raise ValueError('parent final-result SHA-256 is not the pinned p=.02 result')
...
def _validate_experiment_paths(
	...
	if settings.parent_final_result != (
		...
		raise ValueError('parent result path is not the pinned p=.02 result')
	...
	if settings.runs_root != validation_root / 'runs':
		raise ValueError('runs_root is outside the isolated Z-filter namespace')
	if (
		settings.protocol_lock
		!= validation_root / 'zero_phase_z_filter_protocol_lock.json'
	):
		raise ValueError('protocol lock path changed')
	if (
		settings.final_result
		!= validation_root / 'zero_phase_z_filter_final_result.json'
	):
		raise ValueError('final result path changed')


def validate_parent_result(  # noqa: C901, PLR0912
	settings: ValidationSettings, canonical: F3FiveWayConfig
) -> dict[str, object]:
	"""Pin the closed p=.02 arm as attribution-only medium evidence."""
	parent, parent_sha = _read_hashed_json(
		settings.parent_final_result, label='parent trace-drop p=.02 final result'
	)
	if parent_sha != settings.parent_final_result_sha256:
		raise ValueError('parent trace-drop p=.02 final-result SHA-256 changed')
	expected_values: Mapping[str, object] = {
		...
		'base_pretraining_epochs': 1,
		'continuation_epochs': 25,
		'candidate_id': P002_CONTROL_ID,
		'augmentations': P002_AUGMENTATIONS,
		'passed': False,
		'winner_candidate_id': None,
		'authorizes_additional_trace_drop_followup': False,
		'authorized_additional_trace_drop_probability': None,
	...
	parent_protocol = _mapping(parent, 'protocol_lock')
	if parent_protocol.get('sha256') != PARENT_PROTOCOL_LOCK_SHA256:
		raise ValueError('parent p=.02 protocol-lock SHA-256 changed')
	if parent_protocol.get('path') != str(
		canonical.artifact_root / PARENT_PROTOCOL_LOCK_RELATIVE_PATH
	):
		raise ValueError('parent p=.02 protocol-lock path changed')
	_, live_parent_protocol_sha = _read_hashed_json(
		canonical.artifact_root / PARENT_PROTOCOL_LOCK_RELATIVE_PATH,
		label='parent trace-drop p=.02 protocol lock',
	)
	if live_parent_protocol_sha != PARENT_PROTOCOL_LOCK_SHA256:
		raise ValueError('parent p=.02 protocol-lock file changed')
	...
	gate_values: Mapping[str, object] = {
		...
		'wins_all_5_over_random': False,
	}
	for key, expected in gate_values.items():
		if not _type_sensitive_equal(gate.get(key), expected):
			raise ValueError(f'parent medium gate {key} changed')
	...
	expected_control_cells = {
		(P002_CONTROL_ID, layout_id, 'medium') for layout_id in LAYOUT_IDS
	}
	if (
		len(controls) != 5
		or {_cell_key(row) for row in controls} != expected_control_cells
	):
		raise ValueError('parent does not contain the exact five p=.02 medium cells')
	...
	if (
		len(parent_random) != 5
		or {_cell_key(row) for row in parent_random} != expected_random_cells
	):
	...
	recomputed_arm = cast(
		...
		_shared('_arm_random_result')(
			scores, arm_id=P002_CONTROL_ID, medium_gate_open=False
		),
	)
	if not _type_sensitive_equal(arm_results.get(P002_CONTROL_ID), recomputed_arm):
		raise ValueError('parent p=.02 arm result does not recompute')
	paired_deltas = {
		layout_id: (
			scores[P002_CONTROL_ID][(layout_id, 'medium')]
			- scores[RANDOM_ID][(layout_id, 'medium')]
		)
		for layout_id in LAYOUT_IDS
	}
	if not _type_sensitive_equal(
		gate.get('paired_macro_f1_deltas_over_random'), paired_deltas
	):
		raise ValueError('parent p=.02 medium paired deltas do not recompute')
	return {
		...
		'passed': False,
		'failure_stage': 'medium_5of5',
		'closed_trace_drop_branch': True,
		'parent_protocol_lock': dict(parent_protocol),
		'frozen_medium_control_ids': [P002_CONTROL_ID],
		'frozen_medium_control_inputs': [dict(row) for row in controls],
		'frozen_parent_random_inputs': [dict(row) for row in parent_random],
		'recomputed_parent_arm_result': dict(recomputed_arm),
...
def audit_candidate_base_checkpoint(
	*,
	...
	return {
		...
		'audit_type': 'zero_phase_z_filter_base1_pre_continuation_checkpoint_only',
		'candidate_id': CANDIDATE_ID,
...
def audit_candidate_checkpoints(
	*,
	...
	return {
		'audit_schema_version': CHECKPOINT_AUDIT_SCHEMA_VERSION,
		'audit_type': (
			'zero_phase_z_filter_base1_pre_extraction_lineage_checkpoint_only'
		),
		'candidate_id': CANDIDATE_ID,
		'base_checkpoint': str(candidate.base_checkpoint),
...
def create_zero_phase_z_filter_protocol_lock(
...
def validate_zero_phase_z_filter_protocol_lock(
	settings: ValidationSettings, canonical: F3FiveWayConfig
) -> Mapping[str, object]:
	"""Replay every immutable protocol input against live evidence."""
	stored = _read_regular_json(
		settings.protocol_lock, label='zero-phase Z-filter protocol lock'
	)
	...
	if not _type_sensitive_equal(stored, expected):
		raise ValueError(
			'zero-phase Z-filter protocol lock differs from live frozen inputs'
		)
	return stored
...
def _protocol_lock_payload(  # noqa: PLR0913
	*,
	...
	return {
		...
		'selection_basis': 'fixed_unlabeled_train_crop_zero_phase_z_filter_w025_v1',
...
def _experiment_repository_state() -> dict[str, object]:
	...
	state['zero_phase_z_filter_experiment_file_inventory'] = inventory
	return state
...
def _protocol_identity(
	...
	if protocol.get('protocol_lock_type') != PROTOCOL_LOCK_TYPE:
		raise ValueError('zero-phase Z-filter protocol lock type changed')
	return {
...
def audit_candidate_source(  # noqa: PLR0913
	*,
	...
	return {
		'audit_schema_version': CHECKPOINT_AUDIT_SCHEMA_VERSION,
		'audit_type': 'zero_phase_z_filter_base1_validation_source_lineage',
		'candidate_id': CANDIDATE_ID,
...
def enforce_validation_order(
	*, settings: ValidationSettings, canonical: F3FiveWayConfig, data_size: str
	...
	protocol = validate_zero_phase_z_filter_protocol_lock(settings, canonical)
	if data_size == 'medium':
...
def _validate_medium_random_gate(
	*,
	...
	if require_open and not gate_open:
		raise ValueError(
			'small/large validation is forbidden because the zero-phase Z filter '
			'did not beat '
...
def create_zero_phase_z_filter_final_result(
	settings: ValidationSettings,
	canonical: F3FiveWayConfig,
	*,
	created_at_utc: str | None = None,
	...
	attribution = {
		'zero_phase_z_filter_minus_trace_drop_p002': _medium_control_contrast(
			attribution_scores,
			left_id=CANDIDATE_ID,
			right_id=P002_CONTROL_ID,
			contrast_id=('zero_phase_z_filter_w025_minus_trace_drop_p002_base1'),
		),
	}
	...
	payload = {
		...
		'frozen_control_provenance': {
			...
			'control_candidate_ids': [P002_CONTROL_ID],
			'data_size': 'medium',
		...
		'zero_phase_z_filter_attribution': attribution,
		**decision,
		'created_at_utc': created_at_utc or _utc_timestamp(),
	}
...
def _terminal_decision(
	*, arm_result: Mapping[str, object], gate_open: bool
	...
	"""Resolve success without silently authorizing another view search."""
...
def _expected_candidate_cells(*, medium_gate_open: bool) -> set[tuple[str, str, str]]:
		...
		for data_size in DATA_SIZES:
			...
			for path in (job.metrics_path, job.output_dir / AUDIT_NAME):
				...
				if expected and not path.is_file():
					raise FileNotFoundError(
						f'missing expected zero-phase Z-filter validation evidence: '
						f'{path}'
					)
...
def build_parser() -> argparse.ArgumentParser:
	...
	parser = argparse.ArgumentParser(
		description='Run one validation-only zero-phase Z-filter candidate job.'
	)
...
def main() -> None:
	...
	if args.create_protocol_lock:
		_print_mapping(create_zero_phase_z_filter_protocol_lock(settings, canonical))
		return
	protocol = validate_zero_phase_z_filter_protocol_lock(settings, canonical)
	protocol_identity = _protocol_identity(settings, protocol)
	...
	if args.create_final_result:
		_print_mapping(create_zero_phase_z_filter_final_result(settings, canonical))
		return
