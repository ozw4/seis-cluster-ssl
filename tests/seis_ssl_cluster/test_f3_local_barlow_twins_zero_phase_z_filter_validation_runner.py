"""Decision and provenance tests for the zero-phase Z-filter experiment-114 runner."""
...
RUNNER_PATH = Path(
	...
	'114_local_barlow_twins_zero_phase_z_filter_view_v1/run_validation.py'
)
...
def _evidence(source_id: str, values: list[float]) -> list[dict[str, object]]:
	return [
		...
		for index, (layout_id, value) in enumerate(zip(LAYOUT_IDS, values, strict=True))
	]
...
def test_protocol_payload_freezes_all_random_before_candidate_metrics(
	...
	assert payload['selection_basis'] == (
		'fixed_unlabeled_train_crop_zero_phase_z_filter_w025_v1'
	)
...
def test_medium_control_contrast_is_attribution_only_and_strict(
	...
	result = contrast(
		...
		right_id='control',
		contrast_id='zero_phase_z_filter_minus_control',
	)
...
def test_terminal_decision_does_not_authorize_an_unplanned_followup(
	...
	assert decision['authorizes_additional_view_followup'] is False
	assert decision['authorized_additional_view_configuration'] is None


def test_final_result_attributes_only_against_frozen_p002_controls(
	runner: Mapping[str, object],
	tmp_path: Path,
	monkeypatch: pytest.MonkeyPatch,
) -> None:
	creator = cast('Any', runner['create_zero_phase_z_filter_final_result'])
	globals_ = creator.__globals__
	controls = _evidence(
		'local_barlow_twins_horizontal_trace_drop_p002_base1ep',
		[0.55] * 5,
	)
	random = _evidence('random', [0.5] * 5)
	candidate = _evidence(CANDIDATE_ID, [0.6] * 5)
	protocol = {
		'parent_result': {
			'path': '/parent/final.json',
			'sha256': 'a' * 64,
	...
	monkeypatch.setitem(
		globals_,
		'_protocol_identity',
		lambda *_: {'path': '/protocol.json', 'sha256': 'b' * 64},
	)
	monkeypatch.setitem(globals_, '_validate_medium_random_gate', lambda **_: gate)
	monkeypatch.setitem(
		globals_, '_validate_exact_candidate_cell_set', lambda **_: None
	)
	monkeypatch.setitem(
		globals_,
		'_candidate_config',
		lambda *_args, **_kwargs: SimpleNamespace(
			model_by_id=lambda _candidate_id: object()
		),
	)
	monkeypatch.setitem(globals_, 'audit_candidate_source', lambda **_: {})
	monkeypatch.setitem(
		globals_, '_validation_order_provenance', lambda *_args, **_kwargs: {}
	)
	monkeypatch.setitem(
		globals_,
		'five_way_runner',
	...
	candidate_by_layout = {row['layout_id']: row for row in candidate}
	monkeypatch.setitem(
		globals_,
		'_read_candidate_job_evidence',
		lambda *, job, **_kwargs: candidate_by_layout[job.layout_id],
	)
	monkeypatch.setitem(
		globals_,
		'_shared',
		lambda name: (
			(
				lambda *_args, **_kwargs: {
					'wins_all_15_over_random': False,
					'positive_delta_count': 0,
				}
			)
			if name == '_arm_random_result'
			else pytest.fail(f'unexpected shared helper: {name}')
		),
	)
	monkeypatch.setitem(
		globals_,
	...
	attribution = result['zero_phase_z_filter_attribution']
	assert set(attribution) == {'zero_phase_z_filter_minus_trace_drop_p002'}
	contrast = attribution['zero_phase_z_filter_minus_trace_drop_p002']
	assert contrast['right_candidate_id'] == controls[0]['candidate_id']
	assert contrast['positive_delta_count'] == 5
	assert result['authorizes_additional_view_followup'] is False
...
def runner_parent_pins_are_exact(text: str) -> bool:
	"""Return whether the closed p=.02 parent identities are source-pinned."""
	return all(
		value in text
		for value in (
