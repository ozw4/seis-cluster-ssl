...
EXPERIMENT_ROOT = (
	...
	'111_local_barlow_twins_gaussian_view_v1/40_base5ep'
)
...
def test_base5_config_is_strict_parent_bound_and_namespaced(
...
def test_parent_audit_reads_selection_payload_not_only_final_summary(
	...
	identity = cast('Any', runner['validate_parent_result'])(local, canonical)
	assert (
	...
	with pytest.raises(ValueError, match='SHA-256 changed'):
		cast('Any', runner['validate_parent_result'])(local, canonical)
...
def test_base_checkpoint_requires_fresh_stage1_and_rejects_lineage(
	...
	payload = _checkpoint_payload(
		runner, config=candidate, epochs=5, continuation_lineage=None
	)
...
def test_protocol_and_selection_payloads_bind_parent_without_base5_metrics(
	runner: dict[str, object],
) -> None:
	parent = {
		'path': '/parent.json',
	...
	assert protocol['stage_boundary'] == 'completed_fresh_base5_before_continuation'
	assert protocol['base_pretraining_epochs'] == 5
	...
	assert selection['base5_metric_inputs'] == []
...
def test_lock_creation_rechecks_for_late_evidence_before_publication(
	...
	protocol_creator = cast('Any', runner['create_base5_protocol_lock'])
	...
	selection_creator = cast('Any', runner['create_base5_selection_lock'])
	selection_checks = 0
	...
	monkeypatch.setitem(
		runtime, 'validate_base5_protocol_lock', lambda *_args: protocol
	)
	monkeypatch.setitem(
		runtime,
		'_protocol_identity',
...
def test_medium_gate_and_exact_reached_cell_sets_are_strict(
	...
	closed = cast('Any', runner['_expected_base5_cells'])(medium_gate_open=False)
	opened = cast('Any', runner['_expected_base5_cells'])(medium_gate_open=True)
...
def test_final_winner_rule_requires_strict_15_of_15(runner: dict[str, object]) -> None:
	chooser = cast('Any', runner['_choose_base5_winner'])
	assert (
...
def test_closed_final_result_records_exact_ten_cells_and_authorizes_base1(
	runner: dict[str, object],
	settings: Any,
	monkeypatch: pytest.MonkeyPatch,
	tmp_path: Path,
	...
	monkeypatch.setitem(
		runtime, 'validate_base5_protocol_lock', lambda _settings, _canonical: protocol
	)
	monkeypatch.setitem(
		runtime,
	...
	local = replace(settings, final_result=tmp_path / 'base5_final.json')
	result = creator(
		local,
		SimpleNamespace(),
		created_at_utc='2026-08-30T00:00:00Z',
	...
	assert result['authorized_next_base_pretraining_epochs'] == 1
