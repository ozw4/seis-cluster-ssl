"""Parity contracts for the reached F3 Local BT five-base-epoch producers."""
...
def test_base5_changes_only_output_and_epoch(
	_model_id: str,
	arm: str,
	...
	assert candidate['train']['epochs'] == 5
...
def test_base5_continuation_is_exact_fixed_25_epoch_lineage(
	_model_id: str,
	arm: str,
...
def test_base5_extraction_uses_matching_checkpoint_and_model_id(
	model_id: str,
	_arm: str,
	_parent_base_path: Path,
	_base_path: Path,
...
def test_base5_producer_outputs_are_pairwise_distinct() -> None:
	outputs: set[str] = set()
	...
	assert all('/base5ep/' in output for output in outputs)
	discovered_paths = {
		path
		for stage_dir in ('10_stage1', '15_stage2', '20_embeddings')
		for path in (BASE5_ROOT / stage_dir).rglob('*.yaml')
	}
