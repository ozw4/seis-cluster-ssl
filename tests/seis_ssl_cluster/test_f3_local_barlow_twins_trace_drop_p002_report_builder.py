"""Report-only projections for F3 trace-drop p=.02 experiment 113."""
...
REPORT_PATH = Path(
	...
	'113_local_barlow_twins_trace_drop_p002_view_v1/build_report.py'
)
CANDIDATE_ID = 'local_barlow_twins_horizontal_trace_drop_p002_base1ep'
P001_ID = 'local_barlow_twins_horizontal_trace_drop_p001_base1ep'
...
def _final_result(*, root: Path, gate_open: bool) -> dict[str, object]:
	...
	controls = [
		_candidate_row(P001_ID, layout, 'medium', 0.55, root=root)
		for layout in LAYOUT_IDS
	]
	return {
...
def test_report_rows_distinguish_live_random_and_frozen_controls(
	...
	assert {
		...
		(CANDIDATE_ID, 'live_trace_drop_p002'),
		('random', 'protocol_frozen_random'),
		(P001_ID, 'frozen_parent_p001_medium_control'),
	}
	control_pairs = [
		row
		for row in paired
		if row['right_source_id'] == P001_ID
	]
...
def test_report_schema_includes_evidence_origin_and_no_pipeline_inputs(
	...
	assert 'medium_p001_macro_f1_mean' in cast(
		'tuple[str, ...]', report['ATTEMPT_FIELDS']
	)
	assert 'medium_delta_vs_p001_mean' in cast(
		'tuple[str, ...]', report['ATTEMPT_FIELDS']
	)
	assert filenames == (
		'attempts.csv',
