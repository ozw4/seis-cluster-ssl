"""Decision and provenance tests for the experiment-112 validation runner."""
...
RUNNER_PATH = Path(
	...
	'112_local_barlow_twins_trace_drop_view_v1/run_validation.py'
)
...
@pytest.mark.parametrize(
	('passed', 'gate_open', 'failure_stage', 'fallback'),
	[
		(True, True, None, None),
		(False, False, 'medium_5of5', 0.02),
		(False, True, 'final_15of15', 0.02),
	],
)
def test_terminal_decision_alone_authorizes_documented_fallback(
	runner: Mapping[str, object],
	...
	assert decision['authorizes_trace_drop_p002_followup'] is (not passed)
	assert decision['authorized_trace_drop_probability'] == fallback
