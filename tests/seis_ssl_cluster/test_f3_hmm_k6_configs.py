...
def artifact_root(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
	...
	for variant, source_method in SOURCE_METHODS.items():
		checkpoint = (
			root
...
def test_hmm_k6_configs_share_the_f3_paired_single_head_contract(
	...
	expected_manifests = {
		'train': str(
			artifact_root
			/ 'registry/manifests/f3/facies_benchmark_v1'
	...
	for run in RUNS:
		...
		for config in (mae, barlow_twins):
			...
			assert config['manifests'] == expected_manifests
			assert 'train_npy_paths.txt' not in config['manifests'][
				'train_path_list'
			]
...
def test_hmm_k6_configs_match_stage1_geometry_and_parihaka_science(
	artifact_root: Path,
) -> None:
	del artifact_root
	for variant, source_method in SOURCE_METHODS.items():
		for run, filename in RUNS.items():
			f3 = load_config(HMM_ROOT / variant / 'hmm/k6' / filename)
			parihaka = load_config(
				PARIHAKA_HMM_ROOT / variant / 'hmm/k6' / filename
			)
			stage1 = load_config(
				STAGE1_ROOT / source_method / STAGE1_RUNS[run]
			)

			for section in ('data', 'zero_mask', 'model'):
				assert f3[section] == stage1[section]
			for section in ('data', 'zero_mask', 'model', 'head', 'loss', 'train'):
				assert f3[section] == parihaka[section]
...
def test_hmm_k6_sources_never_use_controls_or_external_experiments(
	hmm_configs: dict[str, dict[str, dict[str, object]]],
) -> None:
	for variant in VARIANTS:
		for run in RUNS:
			...
			for value in inputs:
				lower = value.lower()
				assert 'mae_continue/full_25ep' not in lower
				assert 'bt_continue/full_25ep' not in lower
				assert 'best.pt' not in lower
				assert 'nopims' not in lower
				assert 'parihaka' not in lower
				assert '/stage2/' not in lower
...
def test_hmm_k6_output_roots_are_unique_and_isolated(
	...
	stage1_and_control_outputs = {
		...
		for stage in (
			...
			'stage2/mae100/mae_continue/gpu_feasibility_1step',
			'stage2/mae100/mae_continue/full_25ep',
			'stage2/bt100/bt_continue/gpu_feasibility_1step',
	...
	target_generation_outputs = {
		artifact_root
		/ namespace
		/ 'f3/facies_benchmark_v1/ssl_hmm_continuation_v1'
		/ suffix
