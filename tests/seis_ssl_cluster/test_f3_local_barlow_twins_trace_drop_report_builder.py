"""Report-only projections for F3 trace-drop experiment 112."""
...
REPORT_PATH = Path(
	...
	'112_local_barlow_twins_trace_drop_view_v1/build_report.py'
)
CANDIDATE_ID = 'local_barlow_twins_horizontal_trace_drop_p001_base1ep'
GAUSSIAN_ID = 'local_barlow_twins_gaussian_noise_std010_base1ep'
...
def _final_result(*, root: Path, gate_open: bool) -> dict[str, object]:
	...
	controls = [
		_candidate_row(source_id, layout, 'medium', value, root=root)
		for source_id, value in ((GAUSSIAN_ID, 0.55), (LEGACY_ID, 0.54))
		for layout in LAYOUT_IDS
	]
	return {
		...
		'trace_drop_attribution': {},
		'passed': gate_open,
		'winner_candidate_id': CANDIDATE_ID if gate_open else None,
		'failure_stage': None if gate_open else 'medium_5of5',
		'authorizes_trace_drop_p002_followup': not gate_open,
		'authorized_trace_drop_probability': None if gate_open else 0.02,
	}


@pytest.mark.parametrize(
	('gate_open', 'expected_rows', 'expected_pairs'),
	[(False, 20, 15), (True, 40, 25)],
)
def test_report_rows_distinguish_live_random_and_frozen_controls(
	...
	assert {
		(row['source_id'], row['evidence_origin']) for row in rows
	} == {
		(CANDIDATE_ID, 'live_trace_drop'),
		('random', 'protocol_frozen_random'),
		(GAUSSIAN_ID, 'frozen_parent_base1_control'),
		(LEGACY_ID, 'frozen_parent_base1_control'),
	}
	control_pairs = [
		row
		for row in paired
		if row['right_source_id'] in {GAUSSIAN_ID, LEGACY_ID}
	]
	assert len(control_pairs) == 10
