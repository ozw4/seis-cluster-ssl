...
SCRIPT = Path(
	...
	'38_channel_hmm_distillation_weight_v1/scripts/summarize_validation.py'
)


def _load_summary_module() -> ModuleType:
	spec = importlib.util.spec_from_file_location(
		'parihaka_hmm_distillation_weight_summary',
		SCRIPT,
	)
...
def _positive_gains() -> dict[str, dict[str, float | tuple[float, ...]]]:
	return {
		'distill005': {'mae': 0.01, 'local_bt': 0.01},
		'distill010': {'mae': 0.04, 'local_bt': 0.04},
		'distill020': {'mae': 0.03, 'local_bt': 0.03},
		'distill040': {'mae': 0.02, 'local_bt': 0.02},
	}
...
def test_known_validation_gains_select_expected_weight_and_write_reports(
	tmp_path: Path,
) -> None:
	...
	assert result['recommended_variant'] == 'distill010'
	assert result['metric'] == 'validation.channel_iou'
	assert result['variant_distillation_weights']['distill010'] == 0.10
	assert result['per_variant']['distill010']['distillation_weight'] == 0.10
	assert result['per_variant']['distill010']['combined']['wins'] == 10
	assert result['per_variant']['distill010']['mae']['layout_gains'][
		'layout_000'
	] == pytest.approx(0.04)
	...
	assert '- Recommended variant: distill010' in written_markdown
...
def test_negative_branch_mean_makes_variant_ineligible(
	...
	gains['distill010'][negative_branch] = -0.01
	existing_root, validation_root, report_root = _write_metrics_matrix(
		tmp_path,
		gains,
	)
	...
	assert result['per_variant']['distill010']['eligible'] is False
	assert 'distill010' not in result['ranking']
	assert result['recommended_variant'] == 'distill020'
...
def test_mismatched_benchmark_identity_is_rejected(tmp_path: Path) -> None:
	...
	with pytest.raises(
		ValueError,
		match=(
			'mae_hmm_k6_distill010/layout_002: '
