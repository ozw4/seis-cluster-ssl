...
def test_embedding_configs_use_paired_f3_stage1_sources(
	...
	for variant, config in embedding_configs.items():
		...
		checkpoint = Path(config['embeddings']['checkpoint'])
		...
		assert Path(config['manifests']['input']) == expected_manifest
...
def test_embedding_configs_are_scientifically_paired(
	...
	assert mae['embeddings']['checkpoint'] != barlow_twins['embeddings'][
		'checkpoint'
	]
	assert mae['embeddings']['output_dir'] != barlow_twins['embeddings'][
		'output_dir'
	]
...
def test_clustering_configs_resolve_as_paired_anchor_only_k6(
	...
	assert mae['paths'] == barlow_twins['paths']
	assert mae['embeddings']['input_dir'] != barlow_twins['embeddings'][
		'input_dir'
	]
	assert mae['clustering']['output_dir'] != barlow_twins['clustering'][
		'output_dir'
	]
	assert {
		key: value
...
def test_export_scripts_are_paired_and_create_one_k6_directory(
	clustering_configs: dict[str, dict[str, object]],
	artifact_root: Path,
) -> None:
	arguments = {
	...
	for variant, args in arguments.items():
		...
		assert export_root.name == variant
		assert export_root.name != 'k6'
...
def test_target_pipeline_does_not_reference_disallowed_sources() -> None:
	for variant in VARIANTS:
		for relative_path in (
			...
			assert 'mae25' not in lowered
			assert 'bt25' not in lowered
			assert '/stage2/' not in lowered
			assert 'nopims' not in lowered
