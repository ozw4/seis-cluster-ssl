...
from seis_ssl_cluster.f3.center_trace_masked_periodic_refresh_screening_audit import (
...
def build_parser() -> argparse.ArgumentParser:
	"""Build the standalone periodic-refresh audit parser."""
	parser = argparse.ArgumentParser(
		description='Audit F3 periodic-refresh original-split screening.'
	)
...
def main() -> int:
	"""Validate or immutably write the periodic-refresh screening evidence."""
	args = build_parser().parse_args()
	result = audit_f3_center_trace_masked_periodic_refresh_screening(
		load_f3_center_trace_masked_periodic_refresh_screening_audit_config(
			args.config
		),
