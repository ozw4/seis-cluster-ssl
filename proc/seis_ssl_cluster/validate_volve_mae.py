'''Thin CLI for validating Volve MAE inputs and training artifacts.'''
...
def build_parser() -> argparse.ArgumentParser:
	'''Build the closed Volve MAE validation parser.'''
	parser = argparse.ArgumentParser(
		description='Validate Volve inputs, CPU smoke, or completed full MAE run.',
	)
...
def main() -> None:
	...
	if args.json_output is not None:
		path = write_volve_mae_validation_report(result, args.json_output)
		print(f'json_output: {path}')


def _print_result(result: VolveMaeValidationResult) -> None:
