...
def main() -> None:
	...
	result = run_section_layout_calibration(
		config,
		mode=str(args.mode),
		dry_run=bool(args.dry_run),
	)
	_print_summary(config, result, mode=str(args.mode), dry_run=bool(args.dry_run))
...
def _print_summary(
	...
	*,
	...
	print(f'section_layout.target_rule: {config.target_rule}')
	print(f'section_layout.dry_run: {str(dry_run).lower()}')
	if mode == 'inspect':
		print(f'section_layout.candidate_rows: {len(payload.get("rows", ()))}')
		...
		print(
			'section_layout.target_train_voxel_counts: '
			f'{payload.get("target_train_voxel_counts")}'
		)
