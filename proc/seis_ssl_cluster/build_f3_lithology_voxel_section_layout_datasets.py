...
DEFAULT_CONFIG = (
	...
	/ 'f3'
	/ 'facies_benchmark_v2'
	/ '109_f3_voxel_section_layout_v2'
...
def main() -> None:
	...
	if args.quarantine_invalid and not args.only_missing:
		parser.error('--quarantine-invalid requires --only-missing')
...
def _print_plan(
	...
	*,
	...
	print(f'condition_count: {len(conditions)}')
	print(f'only_missing: {only_missing}')
	print(f'quarantine_invalid: {quarantine_invalid}')
	for condition in conditions:
		print(
			f'- {condition.layout_id}/{condition.data_size} '  # type: ignore[attr-defined]
			f'target={condition.target_train_voxel_count} '  # type: ignore[attr-defined]
			f'train_voxels={condition.actual_train_voxel_count} -> '  # type: ignore[attr-defined]
			f'{condition.output_dir}'  # type: ignore[attr-defined]
		)
