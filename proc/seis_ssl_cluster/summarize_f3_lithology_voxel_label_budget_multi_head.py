"""Summarize and publish the fixed paired multi-head voxel matrix."""

from __future__ import annotations

from typing import TYPE_CHECKING

from seis_ssl_cluster.cli import (
	build_config_parser,
	load_config_for_cli,
	parse_config_path,
)
from seis_ssl_cluster.config import load_config
from seis_ssl_cluster.config.f3_lithology_voxel_label_budget_multi_head import (
	f3_lithology_voxel_label_budget_multi_head_config_from_mapping,
)
from seis_ssl_cluster.f3.lithology.voxel_label_budget_multi_head_results import (
	inspect_f3_lithology_voxel_label_budget_multi_head_results,
	summarize_f3_lithology_voxel_label_budget_multi_head,
)

if TYPE_CHECKING:
	import argparse


def build_parser() -> argparse.ArgumentParser:
	"""Build the aggregate-stage CLI parser."""
	return build_config_parser(
		'Summarize paired K=6/8/10 multi-head results.',
		config_required=True,
		dry_run_help=(
			'Validate all sources and reaggregate paired metrics without writing.'
		),
	)


def main() -> None:
	"""Validate or materialize the multi-head aggregate."""
	args = build_parser().parse_args()
	raw = load_config_for_cli(parse_config_path(args), loader=load_config)
	if set(raw) == {'run_config'}:
		raw = load_config(raw['run_config'])
	config = f3_lithology_voxel_label_budget_multi_head_config_from_mapping(raw)
	if args.dry_run:
		inspection = inspect_f3_lithology_voxel_label_budget_multi_head_results(config)
		print(f'overall_status: {inspection.decisions["overall_status"]}')
		print('execution: dry-run; summary and publish skipped')
		return
	result = summarize_f3_lithology_voxel_label_budget_multi_head(config)
	print(f'multi_head_results.summary: {result.summary_json}')
	print(f'multi_head_results.status: {result.decisions["overall_status"]}')


if __name__ == '__main__':
	main()
