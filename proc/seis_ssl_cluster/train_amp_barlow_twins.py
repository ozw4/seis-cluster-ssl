"""Thin entrypoint for amplitude-only 3D Barlow Twins pretraining."""
...
DEFAULT_CONFIG = (
	...
	/ 'configs'
	/ 'seis_ssl_cluster'
	/ 'train_amp_barlow_twins.yaml'
)


def build_parser() -> argparse.ArgumentParser:
	"""Build the CLI parser for amplitude Barlow Twins pretraining."""
	parser = build_config_parser(
		'Train an amplitude-only 3D Barlow Twins model.',
	...
	add_path_argument(
		parser,
		'--resume',
		help_text='Resume from a completed-epoch Barlow Twins checkpoint.',
	)
	return parser


def main() -> None:
	"""Run Barlow Twins pretraining or print a dry-run summary."""
	...
	config = resolve_config_for_cli(
		raw_config,
		resolver=resolve_barlow_twins_training_config,
	...
	if args.dry_run:
		...
		return
	checkpoint_path = run_barlow_twins_pretraining(config, resume=args.resume)
