...
EXPECTED_CANDIDATES = {
	...
	'mae100_hmm_v2_mh_k68_distill020': (6, 8),
	'mae100_hmm_v2_mh_k468_distill020': (4, 6, 8),
	'mae100_hmm_v2_mh_k6810_distill020': (6, 8, 10),
}
...
def screening_config_from_mapping(raw: Mapping[str, object]) -> dict[str, object]:
	...
	if raw['control_id'] != 'mae_hmm_k6':
		raise ValueError('control_id must equal mae_hmm_k6')
	entries = raw['candidates']
	if not isinstance(entries, list) or len(entries) != len(EXPECTED_CANDIDATES):
		raise ValueError('screening requires exactly four candidates')
	candidates = []
	for entry in entries:
		_require_exact_keys(
			entry,
			{'candidate_id', 'downstream_config', 'training_config', 'head_ks'},
			'candidate',
		)
		ks = entry['head_ks']
		if (
		...
		candidates.append(
			{
				'candidate_id': _nonempty_string(entry['candidate_id'], 'candidate_id'),
				**{
	...
	for key in ('candidate_id', 'downstream_config', 'training_config'):
		if len({entry[key] for entry in candidates}) != len(candidates):
			raise ValueError(f'duplicate candidate {key}')
	if {
		entry['candidate_id']: tuple(entry['head_ks']) for entry in candidates
	} != EXPECTED_CANDIDATES:
		raise ValueError(
			'candidate IDs and head_ks must match the four screening candidates'
		)
	return {
		...
		'control_freeze_receipt': _absolute_path(
			raw['control_freeze_receipt'], 'control_freeze_receipt'
		),
...
def inspect_multi_head_screening(config: Mapping[str, object]) -> dict[str, object]:
	...
	for entry in config['candidates']:
		...
		if candidate.candidate_id != entry['candidate_id']:
			raise ValueError('candidate ID differs from downstream config')
		...
		_add_metric_deltas(comparison)
		verify_control_freeze_receipt(
			receipt,
			control_audit['checkpoint']['sha256'],
			[
				{
					'layout_id': row['layout_id'],
					'data_size': row['data_size'],
					'mean_iou': row['control_mean_iou'],
		...
		results.append(
			{
				...
				'by_size': {
					size: _aggregate_scope(
						...
						metric='mean_iou',
						delta_key='mean_iou_candidate_minus_control',
					)
				...
				'secondary_macro_f1_by_size': {
					size: _aggregate_scope(
						...
						paired_t_unit='layout_id',
						metric='macro_f1',
						delta_key='macro_f1_candidate_minus_control',
					)
					for size in DATA_SIZES
				},
				...
				'overall_layout_clustered': _aggregate_layout_clustered(
					comparison, delta_key='mean_iou_candidate_minus_control'
				),
			}
		)
	...
	return {
		...
		'control_freeze_receipt': {
			'path': str(config['control_freeze_receipt']),
			'sha256': file_sha256(config['control_freeze_receipt']),
			'receipt': receipt,
		},
...
def summarize_multi_head_screening(
	...
	lines = [
		...
		(
			...
			'(frozen HMM_v1 Condition 2b verified). '
		...
		'',
		'| Candidate | Size | Mean IoU delta | SD | Wins / ties / losses |',
		'| --- | --- | ---: | ---: | --- |',
	]
	for candidate in payload['candidates']:
		for size, aggregate in candidate['by_size'].items():
			delta = aggregate['mean_iou_candidate_minus_control']
			lines.append(
		...
		config['summary_root'],
		{
			'comparison.csv': _comparison_csv(
				...
				fieldnames=(
					*(
						key
						for key in COMPARISON_FIELDNAMES
						if key
						not in (
							'candidate_macro_f1',
							'control_macro_f1',
...
def _add_metric_deltas(rows: list[dict[str, object]]) -> None:
	for row in rows:
		...
		row['macro_f1_candidate_minus_control'] = (
			row['candidate_macro_f1'] - row['control_macro_f1']
		)
		row['mean_iou_candidate_minus_control'] = (
			row['candidate_mean_iou'] - row['control_mean_iou']
		)
		del row['candidate_minus_control']
