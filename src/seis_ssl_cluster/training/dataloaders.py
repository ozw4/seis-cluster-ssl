...
def build_strat_multi_head_target_dataloader(  # noqa: PLR0913
	dataset: object,
	*,
	batch_size: int,
	num_workers: int = 0,
	...
	"""Build a deterministic DataLoader for nested multi-head targets."""
	if num_workers < 0:
		msg = f'num_workers must be nonnegative; got {num_workers!r}'
		raise ValueError(msg)
	generator = torch.Generator()
	...
	return torch.utils.data.DataLoader(
		...
		shuffle=shuffle,
		num_workers=num_workers,
		collate_fn=strat_multi_head_target_collate_fn,
		generator=generator,
		pin_memory=torch_device.type == 'cuda',
		persistent_workers=num_workers > 0,
		worker_init_fn=_make_worker_init_fn(seed),
...
def build_strat_multi_head_posterior_dataloader(  # noqa: PLR0913
	dataset: object,
	*,
	batch_size: int,
	num_workers: int = 0,
	...
	"""Build a deterministic DataLoader for nested soft posterior targets."""
	if num_workers < 0:
		raise ValueError(f'num_workers must be nonnegative; got {num_workers!r}')
	generator = torch.Generator()
	generator.manual_seed(seed)
	torch_device = torch.device(device)
	return torch.utils.data.DataLoader(
		...
		shuffle=shuffle,
		num_workers=num_workers,
		collate_fn=strat_multi_head_posterior_collate_fn,
		generator=generator,
		pin_memory=torch_device.type == 'cuda',
		persistent_workers=num_workers > 0,
		worker_init_fn=_make_worker_init_fn(seed),
