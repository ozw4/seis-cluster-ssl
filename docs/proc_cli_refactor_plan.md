...
| `validate_artifact_paths.py` | local `ArgumentParser`; no YAML | none | `--root`, `--scan`, `--fail-on-runs`, repeated `--allow-pattern`, `--allow-test-fixtures` | direct CLI values only | `validate_artifact_paths` | validation status, counts, findings | none | returns `0`/`1` via `SystemExit(main())` |
